...
namespace Mahanaim
{
    ...
    /// </summary>
    public class JongReader : WaveStream
    {
        ...
        public JongReader(string mp3FileName)
        ...
        public JongReader(string mp3FileName, FrameDecompressorBuilder frameDecompressorBuilder)
        ...
        public JongReader(Stream inputStream)
        ...
        private JongReader(Stream inputStream, FrameDecompressorBuilder frameDecompressorBuilder, bool ownInputStream)
        {
        ...
        private void ValidateFrameFormat(Mp3Frame frame)
        {
            if (frame.SampleRate != Mp3WaveFormat.SampleRate)
            {
                string message =
                    String.Format(
                        "Got a frame at sample rate {0}, in an MP3 with sample rate {1}. JongReader does not support sample rate changes.",
            ...
            if (channels != Mp3WaveFormat.Channels)
            {
                string message =
                    String.Format(
                        "Got a frame with channel mode {0}, in an MP3 with {1} channels. JongReader does not support changes to channel count.",
